...
```
ailc [run|check] [--keep-c|-k] [--json] <input.ail> [output-binary] [-- prog-args]
```
...
- Positional otherwise (arg 2 = output path). `run`/`check`/`test`/`compile` are the only subcommands; there is no `--emit-c`/`--backend`. (The Rust `ailangc run/compile` CLI is a different, archived compiler.)
